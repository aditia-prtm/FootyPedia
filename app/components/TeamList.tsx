...
export default function TeamList({
  teams,
  emptyMessage = "Klub tidak ditemukan.",
  showTips = true,
  maxColumns = 4,
}: TeamListProps) {
  if (!teams || teams.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center border border-dashed border-[#2d2d2d] rounded-2xl px-6 py-16 text-center">
        <ShieldAlert className="h-12 w-12 text-[#3d3d3d] mb-4 stroke-[1.5]" />
        <p className="text-sm font-medium text-[#8a8a8a] mb-1">
          {emptyMessage}
        </p>
        <p className="text-xs text-[#5a5a5a] max-w-xs">
          Periksa ejaan nama atau gunakan nama klub yang lebih pendek.
        </p>
