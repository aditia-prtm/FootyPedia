...
import { TeamInfoGrid } from "@/features/teams";
...
export async function generateMetadata({ params }: TeamPageProps) {
  ...
  if (!team) {
    return { title: `Klub Tidak Ditemukan | ${SITE_CONFIG.name}` };
  }
  return {
    title: `${team.strTeam} - Profil Klub & Skuad Pemain | ${SITE_CONFIG.name}`,
            ...
            <div className="border-b border-[#2d2d2d] pb-5 mb-6">
              ...
              {team.strTeamAlternate && (
                <p className="mt-2 text-xs text-[#5a5a5a]">
                  Alias: <span className="text-[#8a8a8a]">{team.strTeamAlternate}</span>
                </p>
              )}
            </div>
